...
# -*- coding: utf-8 -*-
"""
Tema WordPress FINAL - Solución definitiva
"""
...
def crear_tema_final():
    print("CREANDO TEMA WORDPRESS FINAL...")
...
function sueno_andino_final_setup() {
...
add_action('after_setup_theme', 'sueno_andino_final_setup');

function sueno_andino_final_scripts() {
    wp_enqueue_style('sueno-andino-final-style', get_stylesheet_uri());
    wp_enqueue_style('sueno-andino-final-fonts', 'https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap');
    wp_enqueue_script('jquery');
    wp_enqueue_script('sueno-andino-final-js', get_template_directory_uri() . '/js/theme.js', array('jquery'), '1.0.0', true);
}
add_action('wp_enqueue_scripts', 'sueno_andino_final_scripts');

function sueno_andino_final_create_pages() {
    $pages = array(
...
add_action('after_switch_theme', 'sueno_andino_final_create_pages');
?>
"""
...
""")
    ...
    with open(f"{tema_dir}/js/theme.js", 'w', encoding='utf-8') as f:
        f.write("""// Sueño Andino Final - JavaScript
...
""")
    ...
    zip_filename = f"sueno-andino-final-{timestamp}.zip"
